...
const stickerGroupSchema = new Schema({
	...
	sticker_ids: [{ type: mongoose.Types.ObjectId, ref: 'stickers' }],
	group_id: {
		type: String,
		required: true,
		unique: true,
...
stickerGroupSchema.statics.findOneOrCreate = function (search_params) {
	return this.findOne({ group_id: search_params.group_id })
		.then((object) => {
			console.log(object);
			if (!object) {
		...
		.catch((err) => {
			console.log(
				'Error in findOneOrCreate static function of stickerGroupSchema'
			);
			console.log(err);
		});
};
